...
def generate_from_idx(model, idx, seed=None):
    ...
    encoding_indices = torch.tensor(idx, device=device).unsqueeze(
        1
    )  # torch.randint(0, 17, (16,1), device=device)
    encodings = torch.zeros(encoding_indices.shape[0], num_embeddings, device=device)
    encodings.scatter_(1, encoding_indices, 1)

    # Quantize and unflatten
    quantized = torch.matmul(encodings, embedding.weight).view(
        torch.Size([1, 8, 2, 128])
    )
...
if __name__ == "__main__":
    ...
    if not args.file:
        ...
        for k in range(16):
            if not k:
                codebooks_idx = gpt_model.generate(
                    x,
                    191,
                    do_sample=True,
                    top_k=args.top_key,
        ...
        x = torch.tensor(
            ludovico_vae.codebooks2vocab(model, tune_name=args.file), device=device
        ).unsqueeze(0)
        generated = []
        for k in range(16):
            if not k:
    ...
    for c in generated:
        new = np.round(
            generate_from_idx(model, c, seed=None).data.cpu().detach().numpy().squeeze()
        )
    ...
    name = (
        f"data/generated/gen_{first_idx}_temperature_{temp}_top_key_{args.top_key}.mid"
    )
    gen.save(name)
