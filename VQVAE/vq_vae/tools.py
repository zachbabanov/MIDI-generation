...
class LudovicoVAE:
    ...
    def set_model(
        ...
        with open(
            os.path.join(self.work_dir, f"{self.config_name}.json"), "w"
        ) as outfile:
    ...
    def get_model(self, state_dict_name="last"):
        ...
        model.load_state_dict(
            torch.load(
                os.path.join(self.work_dir, f"state_dict/{state_dict_name}"),
                map_location="cpu",
            )
        )
        return model
    ...
    def codebooks2vocab(self, model, seq_len=192, tune_name=""):
        ...
        for tune in tqdm(tunes):
            ...
            quarters = (
                torch.tensor(np.reshape(np.array(quarters), (len(quarters), 1, 88, 32)))
                .to(torch.float32)
                .to(self.device)
            )
            ...
            if tune_name:
                ...
                slices = np.array(
                    [
                        codebooks_idx[i * 16 : i * 16 + seq_len]
                        for i in range(((codebooks_idx.shape[0] - seq_len) // 16) + 1)
                    ]
                )
...
class TrainerVQVAE:
        ...
        for k, tune in enumerate(tunes):
            ...
            if augmentation:
                for n in range(3):
                    ...
                    augmented_quarters = list(
                        map(self.augmentation, quarters_to_extend, axis)
                    )
                    for a_q in augmented_quarters:
                        quarters.extend(a_q)
            self.training_data.extend(quarters)
            print(
                f"\rcreating training dataset. progress: {((k+1)/len(tunes)*100):.2f}%",
                end="",
            )
    ...
    def get_train_loader(self):
        ...
        self.validation_loader = DataLoader(
            self.validation_data, batch_size=128, shuffle=True, pin_memory=True
        )
    ...
    def train(self, learning_rate=1e-3, epochs=10):
        ...
        for i in range(num_training_updates):
            ...
            print(
                f"\repoch: {i//self.training_data.__len__()} | progress: {((i+1)%self.training_data.__len__())/self.training_data.__len__()*100:.2f}% | recon_error: {np.mean(train_res_recon_error[-100:]):-4f} | perplexity : {np.mean(train_res_perplexity[-100:]):.4f}",
                end="",
            )
            ...
            if not i % self.training_data.__len__():
                ...
                with torch.no_grad():
                    ...
                    for i_val in range(len(self.validation_data)):
                        valid_originals = next(iter(self.validation_loader)).to(
                            torch.float32
                        )
                        valid_originals = valid_originals.to(self.device)[0].unsqueeze(
                            0
                        )
                        vq_output_eval = self.model._pre_vq_conv(
                            self.model._encoder(valid_originals)
                        )
                        _, valid_quantize, _, encodings = self.model._vq_vae(
                            vq_output_eval
                        )
                        valid_reconstructions = self.model._decoder(valid_quantize)
                        pred = np.round(
                            valid_reconstructions.data.cpu().detach().numpy().squeeze()
                        )
                        true = np.round(
                            valid_originals.data.cpu().detach().numpy().squeeze()
                        )
                    ...
                    print(
                        f"validation recon_error: {np.mean(valid_res_recon_error):.4f}\n"
                    )
                    if np.mean(valid_res_recon_error) < best_score:
                        torch.save(
                            self.model.state_dict(),
                            f"vq_vae/{self.config_name}/state_dict/best",
                        )
                        best_score = np.mean(valid_res_recon_error)
            torch.save(
                self.model.state_dict(), f"vq_vae/{self.config_name}/state_dict/last"
            )
