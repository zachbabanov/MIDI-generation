...
x = Conv2DTranspose(
    num_channels, 3, padding="same", activation="sigmoid", name="decoder_output"
)(x)
...
for epoch in range(EPOCHS):
    ...
    history_callbacks = vae.fit(
        x_train, None, epochs=1, batch_size=32, validation_split=0.2
    )
    ...
    ftxt = open(logs_path + "\\" + "loss_logs" + ".txt", "a")
    ftxt.write(str(loss_history[0]) + "," + str(val_loss_history[0]) + "\n")
    ftxt.close()
    # np.savetxt("loss_history.txt", numpy_loss_history, delimiter=",")
    if True:  # epoch % 2 == 0 or epoch == 9:
        vae.save("model_dir\\" + "\\vae_%03d.h5" % (epoch + 1))
        decoder.save("model_dir\\" + "decoder_%03d.h5" % (epoch + 1))
        plt.close()
        ...
        plt.ylabel("dim 2")
        plt.colorbar()
        plt.savefig("results\\plots\\" + "space_" + str(epoch + 1))
...
for i, yi in enumerate(grid_y):
    for j, xi in enumerate(grid_x):
        ...
        figure[
            i * img_width : (i + 1) * img_width, j * img_height : (j + 1) * img_height
        ] = digit
